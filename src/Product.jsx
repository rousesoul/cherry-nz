...
import { post, get, put, remove } from "./services/http";
...
function Product() {
  ...
  const onRowAdd = (newData, resolve) => {
    post(api.apiURL + api.productCreate, ProductService.productColumns(newData))
      .then((response) => {
  ...
  function onRowUpdate(newData, oldData, resolve) {
    put(api.apiURL + api.productUpdate, ProductService.productColumns(newData))
      .then((response) => {
    ...
    currentUser
      ...
      <div className="mt-3">
        <MaterialTable
          ...
          actions={[
            {
              icon: () => <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-upload" viewBox="0 0 16 16">
